...
pub fn init(device: *rhi.Device, signaled: bool) !Fence {
...
pub fn wait_for_fences(comptime reserve: usize, device: *rhi.Device, fences: []const *Fence) !void {
    if (rhi.is_target_selected(.vk)) {
...
pub fn wait_for_fences_alloc(allocator: std.mem.Allocator, device: *rhi.Device, fences: []const *Fence) void {
    if (rhi.is_target_selected(.vk)) {
